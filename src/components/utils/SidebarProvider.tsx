...
export interface ComponentSidebarItem extends BaseSidebarItem {
  component: ReactNode;
  navigate?: never;
}

export interface NavigateSidebarItem extends BaseSidebarItem {
  navigate: string;
  component?: never;
}

export type SidebarItem = ComponentSidebarItem | NavigateSidebarItem;
...
export const SidebarProvider = ({ children, config }: SidebarProviderProps) => {
  ...
  const topItems = config.filter(
    (item: SidebarItem) => item.position === "top" || !item.position,
  );
  const bottomItems = config.filter(
    (item: SidebarItem) => item.position === "bottom",
  );

  const activeItem = config.find(
    (item: SidebarItem) => item.id === activeComponentId,
  );
